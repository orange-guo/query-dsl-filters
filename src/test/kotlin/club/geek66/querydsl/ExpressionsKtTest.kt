...
internal class ExpressionsKtTest {
	...
	fun generateExpression() {
		...
		val expressions: Set<BooleanExpression> = generateExpressions(
			QUser.user,
			setOf(QueryDslPathBinding(source = UserDto::jobIndustryName.nel(), target = QUser.user.job.industry.name)),
			setOf(PathExpFilter(path = "jobIndustryName", operator = "=", "Java"))
